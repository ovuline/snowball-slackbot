...
const self = module.exports = {
    ...
    orgList.forEach((orgName) => {
      ...
      promiseArr.push(
        new Promise((resolve, reject) => {
          ...
          client.org(orgName).repos({per_page:100}, (err, data, headers) => {
            if (err) {
              reject(err);
            } else {
              for (let i in data) {
  ...
  getMostRecentReviews: (data) => {
    ...
    const reviewPRURLs = data.map(item => item.pull_request_url);

    const uniqueReviewPRURLs = _.uniq(reviewPRURLs);
    // For each unique review PR URL, get the most recent review for that URL.
    for (let i in uniqueReviewPRURLs) {

        const reviews = [];

        // Get the 1 or more PRs with that URL.
        const prsWithUniqueReviewPRURL = data.filter(review => review.pull_request_url == uniqueReviewPRURLs[i]);

        // Get the most recent of those reviews.
        const mostRecentPRWithUniqueReviewPRURL = prsWithUniqueReviewPRURL.sort(function(a,b) {
            return new Date(b.submitted_at).getTime() - new Date(a.submitted_at).getTime()
        })[0];

        reviews.push(mostRecentPRWithUniqueReviewPRURL);

        // For each of these most recent reviews, push some relevant information about them to a new obj.
