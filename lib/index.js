  ...
  startBot: (data) => {
    bot.on('start', (data) => {
      ...
      init = () => {
        ...
        const timezoneTimeString = moment.tz(now, "America/New_York").format().slice(0,-6);
        ...
        console.log(`What is going on - now: ${now}, hours: ${hours}, day: ${day}`);

        // Only execute the poller when it's during working hours on weekdays
        const isMorningPingTime = (hours == 9 && (parseInt(minutes / 10) == 3));
        const isAfterNoonTime = (hours == 13 && (parseInt(minutes / 10) == 3));
        if (validWorkday && (isMorningPingTime || isAfterNoonTime)) {
            getOpenPullRequests(orgs);
