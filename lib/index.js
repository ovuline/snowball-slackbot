    ...
    bot.on('start', (data) => {
      ...
      parsePullRequests = (githubUserNames, now, orgs, githubUser) => {
        github.allOrgsRepos(orgs).then(orgRepos => {
          ...
          messageAuthors(users, parsedPulls, now, githubUser); // puts the matched pulls into a dictionary structure.
        }).catch((err) => {
          console.log(err);
        });
      };
      ...
      init = () => {
        ...
        for (var timezoneKey in timezones) {
            ...
            console.log(`key - ${timezoneKey} value - ${timezones[timezoneKey]}`)
            ...
            const isMorningPingTime = true;//(hours == 9 && (parseInt(minutes / 10) == 3));
            ...
            if (validWorkday && (isMorningPingTime || isAfterNoonPingTime)) {
                ...
                getReviewersRequested([], timezoneDate, orgs);
                // Sends a message to authors.
                parsePullRequests([], timezoneDate, orgs);
            } else {
                console.log('All work and no play makes Jack a dull boy.');
            }

            // Polls every 10 min.
            setTimeout(init, 2 * 60 * 1000);
